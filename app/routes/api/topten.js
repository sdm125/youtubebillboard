...
const doesChartExist = (date) => {
  return db
    ...
    .where(
      'start_date',
      db.raw('date_sub(?, interval dayofweek(?) - 1  day)', [date, date])
    )
    .limit(1);
};
    ...
    doesChartExist(date).then((data) => {
      if (data.length) {
        db.select('s.title', 'a.name as artist', 'c.rank')
          ...
          .andWhere(
            'c.end_date',
            db.raw('date_add(?, interval 7 - dayofweek(?) day)', [date, date])
          )
          .orderBy('c.rank')
          .limit(10)
          .offset(page * 10 - 10)
...
const storeSongsInDB = (songs, date) => {
  let inserts = songs.reduce(
    (inserts, song) => {
      inserts.songs.push({
        title: song.title,
        artist_id: db.raw('(select id from artists where name = ? limit 1)', [
          song.artist,
        ]),
      });
      ...
      inserts.chart.push({
        ...
        rank: song.rank,
        start_date: db.raw('date_sub(?, interval dayofweek(?) - 1  day)', [
          date,
          date,
        ]),
        end_date: db.raw('date_add(?, interval 7 - dayofweek(?) day)', [
          date,
          date,
        ]),
      });
  ...
  db.raw(
    `select *
      from (
        ...
        c.end_date as date
        from charts as c
        left join artists as a on c.artist_id = a.id
        left join songs as s on c.song_id = s.id
